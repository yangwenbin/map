(function() {
  ...
  z.ma = bdmapcfg.home; /*hm 改成离线的路径 */
	...
	function Qb(a, b) {
    if (/^http/.test(a)) return; /*hm 这里判断一下，如果是调用外部资源就退出去好了 */
		if (b) {
	...
	var Rb = { /*hm 这是加载模块的名字 map_tk0ryi  http://api0.map.bdimg.com/getmodules?v=2.0&mod=模块1,模块2 */
			...
			if (!this.map.Ub() && (a ? this.map.K.s_ = a : a = this.map.K.s_, a)) for (var c = p, c = "2" == z.ou ? [z.url.proto + z.url.domain.main_domain_cdn.other[0] + "/"] : [z.url.proto + z.url.domain.main_domain_cdn.baidu[0] + "/", z.url.proto + z.url.domain.main_domain_cdn.baidu[1] + "/", z.url.proto + z.url.domain.main_domain_cdn.baidu[2] + "/"], d = 0, e; e = this.Km[d]; d++) if (e.f_ == o) {
				...
				e.getTilesUrl = function(b, d) {
					...
					if(e<yangmin){
						yangmin=e;
						console.log(l);
						console.log("min "+yangmin);
					}
					
					if(k>yangmaxy){
						yangmaxy=k;
						console.log("maxyy "+yangmaxy);
					}
					
					if(k<yangminy){
						yangminy=k;
						console.log("minyy "+yangminy);
					}
	...
	Vc.getTilesUrl = function(a, b, c) {
      ...
      //ywb20190510加开始
      if(bdmapcfg.tiles_dir=="none"){
    	  return;
      }
      //ywb20190510加结束
      
      return tdir + "/" + b + "/" + d + "/" + a + bdmapcfg.imgext; /*hm 使用本地的瓦片 */
      /*
			c = Uc[c];
		this.map.dE() && (e = 2);
